# 多MCU自动化编程智能体 — 总体规划 v2.3（产品名：FirmForge / 命令行 ff）

> 版本：v2.3 | 日期：2026-07-08 | 更新：2026-07-11（竞品 6 仓库借鉴融合 + 7-Stage Pipeline + 文件格式约定）| 状态：board+vendor 架构定型，AHL 退役
>
> **🏷️ 产品命名（v2.3 锁定）**：英文 **FirmForge**，命令行 `ff`，Python 包 `firmforge`（产品名全部统一，类比 platformio/flask/django 惯例）。中文名暂定。
>
> **v2.3 相对 v2.2 的关键变更**：
> - **AHL 退役**：AHL 作为代码层名字完全下线，后期不再使用。设计原则转为"BSP 设计准则"。
> - **board 顶层架构**：以电路板为代码组织顶层单位（`boards/<board>/`），vendor 芯片系列库被引用复用不复制。
> - **裸工具链**：不用 PlatformIO 平台，Provider 直接调用裸工具 CLI（avr-gcc+avrdude / arm-none-eabi-gcc+openocd），全免费无授权。
> - **吸收知识库协议与设计原则**：将《知识库协议接口定义》P0/P1/P2 改进项与《AHL 设计原则》5 补丁并入本文档。
> - **Arduino 优先单线**：首发 Arduino Mega2560，STM32 推后至验证点通过后启动。
>
> **v2.2 相对 v1.0 的变更（保留）**：首发从 STC8H 调整为 Arduino+STM32；STC8H 降级后期；纳入 5 项补充设计。
>
> 📜 **文档治理声明（最高纲领）**：本规划文档（v2.3）是 FirmForge 项目**唯一最高纲领性文件**。所有架构、命名、模块归属、关键决策以本文为准。原《知识库协议接口定义》及评审报告、《AHL 设计原则》等前期文档已统一移入 `docs/历史/` 目录，**仅作历史参考，不再作为开发准则**；若其与本文冲突，以本文为准。各模块的详细设计（接口、数据结构、算法）在开发该模块时再单独约定，不前置固化。
>
> 关联文档（仅参考，非准则）：`docs/历史/多MCU自动化编程智能体-总体规划.md`（v1.0）、`docs/历史/方案评价报告.md`、`docs/历史/知识库协议接口定义.md`、`docs/历史/知识库协议接口定义-评审报告.md`、`docs/历史/AHL设计原则-评价报告.md`、`docs/历史/综合分析评估报告.md`、`docs/历史/竞品对比分析-6仓库横向对比.md`（6 仓库竞品分析，P0/P1 已吸收进 §2.8/§3.1/§3.4/§4.2）、`.workbuddy/PROJECT_RULES.md`、`.workbuddy/skills/SKILLS.md`
...
设计端到端自动化编程智能体，用户以自然语言描述需求（含板级信息），智能体按 **FirmForge 7-Stage Pipeline**（§2.9）自动完成**初始化 → 规划 → 代码生成 → 编译 → 烧录 → 硬件在环测试 → 验证**的完整闭环。**首发 Arduino Mega2560 单线**，验证点通过后启动 STM32F103 线，后期按板子扩展。
...
智能体核心层 (Agent Core)   BoardDetector → PlanGenerator → CodeGenerator → ToolOrchestrator + Skill Engine + Agent Trace(执行轨迹/工作记忆) + Experience Ledger(经验账本/跨会话) + ContextManager(三层Token)
...
Arduino: apps/ → boards/arduino_mega/ + vendor/arduino/ → AVR 寄存器
         （直接用 Arduino API + variant，无独立封装层）
STM32:   apps/ → boards/stm32f103vet6_minisys/ + vendor/stm32/ → HAL → 寄存器
         （board 级放引脚配置，vendor 级放 BSP 基类 + HAL）
```

> 约束：AI 生成的应用代码调用板级 API（`bsp_*`）或原厂 API（Arduino: `pinMode`；STM32: `HAL_GPIO_*`），不直接操作寄存器（性能关键场景经审批可 escape）。
