...
**FirmForge —— AI 写的 MCU 代码，过寄存器门禁，上真实硬件验证。**

### 生态位

FirmForge 不是 Agent。它是被 Agent 调用的**验证工具链**。
...
| 功能 | 竞品存在 | 说明 |
|------|---------|------|
| 寄存器引用门禁（编译前） | **无** | 编译前拦截 LLM 幻觉寄存器 |
| 配置合理性评分 | **无** | 波特率误差、引脚冲突检测 |
| 裸工具链编译 | PlatformIO | 零许可费，可扩展多平台 |
| CH340 驱动全版本自适应 | **无** | pyserial→Win32Serial fallback |
| 烧录 + 串口行为闭环验证 | **无** | 烧录后自动读串口做模式匹配 |
