...
struct _FirmForgeBoot {
    _FirmForgeBoot() {
        ...
        UCSR0B = (1 << TXEN0);
        UCSR0C = (3 << UCSZ00);
        const char* sig = "FIRMFORGE:board=arduino_uno chip=atmega328p F_CPU=16000000 build=c93fe8e3\n";
...
void uart_driver_init(void) {
    ...
    Serial.println(F("UART Counter Started"));
...
void uart_driver_send_counter(unsigned long value) {
...
void loop(void) {
    ...
    uart_driver_send_counter(counter);
