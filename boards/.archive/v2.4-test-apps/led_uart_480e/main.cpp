...
struct _FirmForgeBoot {
    _FirmForgeBoot() {
        ...
        UCSR0B = (1 << TXEN0);
        UCSR0C = (3 << UCSZ00);
        const char* sig = "FIRMFORGE:board=arduino_uno chip=atmega328p F_CPU=16000000 build=480e9d17\n";
 ...
 * 
 * This function blocks for 2 seconds between messages.
 * For non-blocking operation, consider using millis() based timing.
 */
 ...
 * 
 * Main application loop that handles LED blinking and serial output.
 * Both operations are sequential with blocking delays.
 */
void loop(void)
{
    /* Update LED blink state (200ms on, 200ms off) */
    led_blink_update();
    
    /* Send "HELLO" message over serial (every 2 seconds) */
