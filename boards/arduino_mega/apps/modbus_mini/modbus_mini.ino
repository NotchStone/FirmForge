/* Modbus RTU Slave (minimal) — FC03/04/06/16, slave 1, regs 0~99, count 1~20 */
#include <Arduino.h>
...
static void handle_frame(uint16_t len) {
    ...
    case 0x03: case 0x04: { /* Read Holding / Input Registers */
        ...
        for (uint16_t i = 0; i < cnt; i++) {
            Serial.write((uint8_t)((src[addr + i] >> 8) & 0xFF));
            Serial.write((uint8_t)(src[addr + i] & 0xFF));
        }
        /* Recompute CRC over the full response */
        uint8_t resp[3 + 40];
